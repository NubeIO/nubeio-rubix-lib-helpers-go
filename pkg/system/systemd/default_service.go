...
const systemctl = "sudo systemctl"
...
//IsInstalled checks if a program is installed
func (ds *DefaultService) IsInstalled() (ok bool, err error) {
	ds.Host.CommandOpts.CMD = "dpkg --get-selections | grep " + ds.Name
	o, ok, err := ds.Host.RunCommand()
	if err != nil {
		return ok, err
	}
	if strings.Contains(o, "install") {
		return true, err
	} else {
		return false, err
	}
...
func (ds *DefaultService) Start() (ok bool, err error) {
	ds.Host.CommandOpts.CMD = systemctl + " start " + ds.Name
	_, ok, err = ds.Host.RunCommand()
	if err != nil {
		return ok, err
	}
	return ok, err
...
func (ds *DefaultService) Stop() (ok bool, err error) {
	ds.Host.CommandOpts.CMD = systemctl + " stop " + ds.Name
	_, ok, err = ds.Host.RunCommand()
	if err != nil {
		return ok, err
	}
